# Helyx — Implementation Plan

**Companion to:** `Helyx_PRD.md` v1.0
**Version:** 1.0
**Date:** 2026-07-20
...
- Empty Spring Boot 4.1 app that runs on `http://localhost:8080/` with a "Hello from Helyx" page.
- Connects to a **host-installed PostgreSQL 17** using `application-dev.yml`. Developer creates the database manually (`createdb helyx`). No Postgres in Docker for local dev.
...
- Developer setup step in README: `createdb helyx`, `createuser helyx --pwprompt`, `GRANT ALL ON DATABASE helyx TO helyx`.
...
- Bootstrap 5 imported, htmx script tag, Alpine script tag, custom CSS file `helyx.css`.
...
### Definition of Done
- With host Postgres running and `helyx` DB created, `./mvnw spring-boot:run` starts the app on port 8080 and shows "Hello".
...
- `docker-compose.yml` with `helyx-app` + `caddy` only. Postgres is installed natively on the Debian host via `apt install postgresql-17`. The app connects to `host.docker.internal:5432` (or the host's LAN IP) — documented in `INSTALL.md`.
- Caddyfile with automatic TLS for `*.helyx.app`.
...
- `INSTALL.md` for MHZ Debian VPS: install Java tooling only if needed for admin scripts, install Postgres 17 via apt, create `helyx` role + `helyx` database, `pg_hba.conf` config for local + Docker bridge access, then `docker compose up -d`.
...
**DoD:** MHZ instance running on a $10/mo Hetzner VPS with Postgres 17 native, Docker running `helyx-app` + `caddy`, wildcard TLS working for `mhz.helyx.app` (or the chosen domain).
...
### DoD
- MHZ runs its first quarterly review cycle in Helyx.
