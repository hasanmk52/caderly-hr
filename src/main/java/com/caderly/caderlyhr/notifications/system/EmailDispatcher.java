...
public class EmailDispatcher {
    ...
    EmailDispatcher(
            ...
            Clock clock,
            @Value("${caderly.email.dispatcher.enabled:true}") boolean enabled) {
        this.outbox = outbox;
    ...
    /**
     * Scheduled entry point. Tests set {@code caderly.email.dispatcher.enabled=false} and call
     * {@link #dispatchPending()} at a known moment rather than racing the scheduler; the bean
     * still exists so they can.
     */
    @Scheduled(
            fixedDelayString = "${caderly.email.dispatcher.poll-interval-ms:30000}",
            initialDelayString = "${caderly.email.dispatcher.poll-interval-ms:30000}")
